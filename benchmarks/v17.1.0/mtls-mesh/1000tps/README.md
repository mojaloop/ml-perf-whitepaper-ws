...
**Transfer / Quote / Discovery — Leg Breakdown** — every leg's mean latency
decomposed hop by hop (payer FSP → switch → payee FSP → back), the
switch-side internal steps behind each hop, and — for transfer — a Kafka
...
microseconds of the k6 transfer mean). Discovery closes almost completely
from directly-measured hops; quote's ~100ms inter-stage remainder is mostly
Kafka queueing on `topic-quotes-post`/`-put`, whose consumer groups export
no lag data in this cluster so it can't be broken out further. The
transfer Kafka-queueing panel's `topic-notification-event` series can spike
to a nonsensical value (rendering as "years") right at ramp-down, when that
topic's message rate briefly drops near zero and the lag÷rate estimator's
denominator does too — a known artifact of that estimation method, not a
real queueing delay.
