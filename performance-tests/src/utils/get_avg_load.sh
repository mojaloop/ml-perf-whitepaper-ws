...
# Path to the Terraform-generated ssh-config (has ProxyJump through bastion).
# Override with: SSH_CONFIG=/path/to/ssh-config get_avg_load.sh
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPO_ROOT="$(cd "$SCRIPT_DIR/../../.." && pwd)"
SCENARIO="${SCENARIO:-500tps}"
SSH_CONFIG="${SSH_CONFIG:-${REPO_ROOT}/scenarios/${SCENARIO}/artifacts/ssh-config}"
echo $SSH_CONFIG
